...
import uy.edu.ort.ae2.util.ComparatorUtils;

import java.util.Comparator;
import java.util.Iterator;

public
class ListaDobleCircularClase<T>
    implements Lista<T>
{

    private
    class NodoListaDobleCircular<T>
    {
        T dato;
        ...
        public
        boolean estaOrdenado(ListaDobleCircularClase<T>.NodoListaDobleCircular<T> inicio,
                             Comparator<T> comparador)
        {
            if (this.sig == inicio)
            {//Es el fin de la iteracion, de la lista
                return true;//Esto seria como una lista de un nodo.
            }
            if (comparador.compare(this.dato, this.sig.dato) >= 0)
            ...
            Comparator<T> comparator)
        {
            ...
            else if (inicioSubLista1 != null)
            {//La primer lista no es vacia
                return inicioSubLista1;
            }
            else if (inicioSubLista2 != null)
            {//La segunda lista no es vacia,se las dejo por claridad
        ...
        private
        NodoListaDobleCircular<T> obtenerMitadDeLaLista()
        {
            //Metodo de hoare
        ...
        public
        NodoListaDobleCircular<T> buscarElemento(final T valor)
        {
            NodoListaDobleCircular<T> nodo = this;

            do
            {
                if (nodo.dato != null && nodo.dato.equals(valor))
                {
                    return nodo;
                }
                nodo = nodo.sig;
    ...
    @Override
    public
    T eliminar(final T valor)
    {
        NodoListaDobleCircular<T> nodo = this.inicio.buscarElemento(valor);
        if (nodo != null && (getLargo() == 1 || nodo == inicio))
        {
            this.eliminarComienzo();
        }
        else if (nodo != null)
        {
            nodo.eliminame();
